/**
 * Provides a BeansImageToggleInfo component that displays the state
...
class ImageToggleInfo extends Component {
	 ...
	 */
	componentDidMount() {
		apiFetch( { path: '/beans/v1/singular-images' } ).then( ( types ) => {
	...
	/**
	 * Gives current state of the “show featured images on…” Beans setting
	...
	postTypeHasSingularImagesDisabled() {
		const post_type  = 'show_featured_image_' + this.props.currentPostType
		console.log(this.props.currentPostType)
		console.log(this.state.typesWithSingularImagesEnabled)
		return ! this.state.typesWithSingularImagesEnabled.includes( post_type );
	}
...
console.log(data);
		...
		apiFetch( putRequest ).then( ( types ) => {
			...
			let statusAnnouncement = sprintf( __( 'Featured images now enabled on %s.', 'beans' ), this.props.currentPostTypeLabel );

			if ( this.postTypeHasSingularImagesDisabled() ) {
				/* translators: %s: the current page type, such as “Pages” or “Posts”. */
				statusAnnouncement = sprintf( __( 'Featured images now disabled on %s.', 'beans' ), this.props.currentPostTypeLabel );
			}
	...
	render() {
		...
		if ( this.state.updating ) {
			/* translators: %s: the current page type, such as “Pages” or “Posts”. */
			let label = sprintf( __( 'Disabling images on %s...', 'beans' ), this.props.currentPostTypeLabel );

			if ( this.postTypeHasSingularImagesDisabled() ) {
				/* translators: %s: the current page type, such as “Pages” or “Posts”. */
				label = sprintf( __( 'Enabling images on %s...', 'beans' ), this.props.currentPostTypeLabel );
			}
		...
		/* translators: %s: the current page type, such as “Pages” or “Posts”. */
		let statusText = sprintf( __( 'Featured images are enabled on %s. ', 'beans' ), this.props.currentPostTypeLabel );
		/* translators: %s: the current page type, such as “Pages” or “Posts”. */
		let ariaText = sprintf( __( 'Disable featured images on all %s?' ), this.props.currentPostTypeLabel );
		let linkText = __( 'Disable images.', 'beans' );
		let newImageState = 0; // Turn featured images off.

		if ( this.postTypeHasSingularImagesDisabled() ) {
			textClass = 'beans-sidebar-label-disabled';
			/* translators: %s: the current page type, such as “Pages” or “Posts”. */
			statusText = sprintf( __( 'Featured images are disabled on %s.', 'beans' ), this.props.currentPostTypeLabel );
			/* translators: %s: the current page type, such as “Pages” or “Posts”. */
			ariaText = sprintf( __( 'Enable featured images on all %s' ), this.props.currentPostTypeLabel );
			linkText = __( 'Enable images.', 'beans' );
...
export const BeansImageToggleInfo = compose( [
	withSelect( () => {
