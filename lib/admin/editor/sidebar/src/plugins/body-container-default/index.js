			...
			<Fragment>
				<Fill name="BeansSidebar">
					<Panel>
						<PanelBody
							initialOpen={true}
							title={__("Default Container Width", "tm-beans")}
						>
							{// console.log(this.state.containerOptions);
							this.state.containerOptions.length ? (
								<SelectControl
									label={__("Select Container Width", "tm-beans")}
...
const render = compose([
	withSelect(() => {
		return {
			container: select("core/editor").getEditedPostAttribute("meta")
				._beans_body_container
		};
	}),
...
registerPlugin("beans-body-container-default", { render });
